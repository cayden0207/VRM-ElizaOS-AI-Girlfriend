...
class AIGirlfriendChatSystemV2 {
    ...
    getAPIBaseURL() {
        // 使用config.js中的配置
        const baseUrl = window.AppConfig?.getApiUrl() || 'http://localhost:3000';
        // 开发环境
    ...
    async initializeMemorySystem() {
        // Memory system handled by ElizaOS backend - no frontend memory needed
        console.log('🧠 Memory system handled by ElizaOS backend');
    }
    ...
    async initializeUser() {
        ...
        if (walletAddress && userId) {
            ...
            try {
                // ElizaOS backend handles authentication differently
                // Just verify user exists in profiles
                const response = await fetch(`${this.apiBaseURL}/api/profiles/${walletAddress}`);
        ...
        const hasStoredWallet = localStorage.getItem('wallet_address') || localStorage.getItem('walletAddress');
    ...
    showWalletRequired() {
        ...
        if (loginOverlay && loginOverlay.style.display !== 'none') {
            setTimeout(() => {
                if (!this.currentUser && !localStorage.getItem('wallet_address') && !localStorage.getItem('walletAddress')) {
    ...
    async initializeCharacterChat() {
        ...
        // 初始化记忆统计
        await this.initializeCharacterMemories();
     ...
     * 初始化角色记忆统计
     */
    async initializeCharacterMemories() {
        if (!this.memorySystem || !this.currentUser || !this.currentCharacter) return;
        ...
        try {
            ...
            console.log(`📊 ${this.currentCharacter.name}的记忆统计:`, stats);
            
            // 如果是首次交互，可以保存一些基础信息
            if (stats.total === 0) {
                await this.saveInitialMemories();
            }
        } catch (error) {
            console.error('❌ 初始化记忆统计失败:', error);
        }
    }
        ...
        try {
            ...
            if (this.currentUser.nickname) {
                await this.memorySystem.saveMemory(
                    this.currentUser.id,
                    this.currentCharacter.id,
                    `用户的昵称是${this.currentUser.nickname}`,
                    {
                        memoryType: 'fact',
                        source: 'profile',
                        immediate: true,
            ...
            await this.memorySystem.saveMemory(
                this.currentUser.id,
                this.currentCharacter.id,
                `今天第一次与用户${this.currentUser.nickname || this.currentUser.id}见面聊天`,
                {
                    memoryType: 'event',
                    source: 'chat',
                    immediate: true,
            ...
            console.log('💾 初始记忆信息已保存');
        } catch (error) {
            console.error('❌ 保存初始记忆失败:', error);
        }
    }
    ...
    async loadChatHistory() {
        ...
        try {
            ...
            if (this.memorySystem) {
                const recentMemories = await this.memorySystem.searchMemories(
                    this.currentUser.id,
                    this.currentCharacter.id,
                    '对话',
                    { limit: 5, threshold: 0.3 } // 优化基于性能测试
                );
                
                // 转换记忆格式为聊天历史格式
                this.chatHistory = recentMemories.map((memory, index) => ({
                    id: memory.id || Date.now() - index,
                    sender: memory.content.includes('用户说:') ? 'user' : 'ai',
                    content: memory.content.replace(/^(用户说:|.*回复:)\s*/, ''),
                ...
                console.log(`📚 从记忆系统加载了${this.chatHistory.length}条聊天记录`);
                return;
            }
    ...
    async sendMessage(message) {
        ...
        try {
            ...
            if (false && window.aiChatIntegration) {
                ...
                console.log('📤 ElizaOS API 请求:', `${this.apiBaseURL}/api/chat`);
                ...
                const data = await response.json();
                console.log('🔍 ElizaOS API 响应状态:', response.status);
                console.log('🔍 ElizaOS API 响应数据:', data);
                ...
                if (data.success && data.data && data.data.response) {
                    ...
                    const aiMessage = {
                        ...
                        expression: data.data.emotion || 'neutral'
                    };
                    
                    this.chatHistory.push(aiMessage);
                    
                    // ElizaOS后端处理记忆系统，前端不需要保存
                    // await this.saveAIResponse(aiMessage.content);
    ...
    async saveUserMessage(message) {
        // ElizaOS backend handles all memory persistence
    }
    ...
    async getMemoryContext(currentMessage) {
        // ElizaOS backend handles all memory context
        return '';
    }
        ...
        try {
            ...
            if (this.memorySystem) {
                ...
                return {
                    ...
                    relationshipLevel: Math.min(5, Math.floor(stats.total / 10) + 1), // 基于记忆数量计算关系等级
