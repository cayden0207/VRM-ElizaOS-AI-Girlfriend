...
import { BridgeError, ErrorType, WSMessageType } from './types.js';
            ...
            try {
                ...
                const characters = await Promise.all(characterIds.map(async (characterId) => {
                    ...
                    return {
                        ...
                        model: status.model,
                        voiceId: this.voiceMapping[characterId]
                    };
                }));
                res.json({
                    success: true,
...
eliza_bridge_response_time_ms ${metrics.averageResponseTime}
    ...
    async handleChatRequest(req, res) {
        ...
        try {
            ...
            if (!userId || !characterId || !message) {
                throw new BridgeError('缺少必需参数: userId, characterId, message', ErrorType.INVALID_REQUEST);
            }
            // 处理消息
     ...
     * 处理 ElizaOS 消息
     */
    async handleElizaMessage(req, res) {
        // 专门的 ElizaOS 接口实现
        await this.handleChatRequest(req, res);
    }
    /**
    ...
    async processMessage(userId, characterId, message, options, requestId) {
        ...
        try {
            ...
            if (this.config.elizaEnabled) {
                const elizaResponse = await this.runtimeManager.processMessage(userId, characterId, message, options);
                // 增强响应
                const enhancedResponse = await this.enhanceResponse(elizaResponse, characterId, options);
            ...
            if (this.config.fallbackEnabled) {
                const fallbackResponse = await this.handleFallback(userId, characterId, message, options, requestId);
     ...
     * 增强响应 - 添加 VRM 动画和语音
     */
    async enhanceResponse(elizaResponse, characterId, options) {
     ...
     * 生成语音 URL (模拟实现)
     */
    async generateVoiceUrl(text, characterId) {
        // 这里应该调用 ElevenLabs API 或其他 TTS 服务
        // 现在返回模拟 URL
        const voiceId = this.voiceMapping[characterId];
        if (!voiceId) {
     ...
     * 降级处理
     */
    async handleFallback(userId, characterId, message, options, requestId) {
        // 模拟降级系统响应
            ...
            switch (message.type) {
                case WSMessageType.CHAT:
                    if (message.characterId && message.data?.message) {
                        const response = await this.processMessage(message.userId, message.characterId, message.data.message, message.data.options || {}, connectionId);
                        ws.send(JSON.stringify({
